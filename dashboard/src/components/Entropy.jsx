...
export function Entropy({ className = "", size = Math.max(window.innerWidth, window.innerHeight) }) {
  ...
  useEffect(() => {
    ...
    // Using white theme for particles over dark background
    ...
    class Particle {
      ...
      update() {
        if (this.order) {
          // Ordered particle influenced by chaos
          ...
          // Blend ordered movement and chaos influence
          ...
          // Influence gradually dissipates
          this.influence *= 0.99;
        } else {
          // Chaotic movement
    ...
    function animate() {
      ...
      // Periodically update neighbors for performance
      if (time % 30 === 0) {
        updateNeighbors();
      }
